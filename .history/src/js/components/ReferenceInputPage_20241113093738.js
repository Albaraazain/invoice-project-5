...
export class ReferenceInputPage {
  ...
  async handleSubmit(event) {
    ...
    try{
      ...
      // Store bill data in sessionStorage before navigation
      sessionStorage.setItem("billData", JSON.stringify(billData));

      // Navigate only after bill data is fetched and stored
      window.router.push("/bill-review");
    } catch (error) {
      this.showError(`Error generating quote: ${error.message}`);
